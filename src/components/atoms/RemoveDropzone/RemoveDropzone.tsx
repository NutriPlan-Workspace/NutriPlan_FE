...
const RemoveDropzone: React.FC = () => {
  ...
  return (
    <div
      ...
      className={cn(
        'flex h-[100px] w-[200px] flex-col items-center justify-center gap-4 rounded-md border-2 border-dashed border-red-200 opacity-0',
    ...
    >
      <DeleteOutlined
        className={cn(
          'text-lg text-red-400 transition-all duration-100',
          isDragEnter && 'text-red-500',
        )}
      />
      <span
        className={cn(
          'text-center text-sm text-red-400 transition-all duration-100',
          isDragEnter && 'text-red-500',
        )}
      >
        Drag here to remove
